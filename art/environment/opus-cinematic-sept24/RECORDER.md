# opus-cinematic-sept24 — the recorder

`record.mjs` renders the whole 30 s film from the built game, headless, on the native GPU. It handles both camera-only shots (Link hidden, broll-style) and shots of the real character: the actual game's `PlayerHandle`, driven by input at 60 Hz.
...
robocopy E:\zeldaremake-opus-cinematic\dist E:\zeldaremake-opus-cinematic\gauntlet\out\opus-cinematic-sept24\dist-rec /MIR

$env:ZR_NATIVE_GPU='1'; $env:CAPSLOT_STALE_MIN='Infinity'; node art/environment/owner-fable-canopy/tools/capslot.mjs opus-cinematic -- node art/environment/opus-cinematic-sept24/record.mjs --shots art/environment/opus-cinematic-sept24/shots.json --dist gauntlet/out/opus-cinematic-sept24/dist-rec --out gauntlet/out/opus-cinematic-sept24/take1 --size 3840x2160 --only 01-hero,02-walk
```
...
## Outputs (`--out`, keep it under `gauntlet/out/opus-cinematic-sept24/`, git-ignored)
...
## Known limits / src hooks that would help

- **No yaw setter on `PlayerHandle`:** Link always starts at yaw π, and the preroll turns him. A `setHeading(yaw)` hook would make the preroll optional.
- **The head-look snap bug** (see wf-critic (a)) shows in idle frames until it is fixed in `glbLink.ts`. For standing Link, choose `time` away from `t ≈ 2(yaw+π) mod 4π`.
- **Stair treads under the camera clamp** come from the layout audit's stair boxes. The ground Link actually walks on (`ground.height`) is not exposed.
