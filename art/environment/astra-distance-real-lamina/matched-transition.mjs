...
export async function captureTransition(page, out) {
  ...
  for (const pass of ['first', 'warm']) {
    ...
    result.passes.push({ name: pass, rows, shots });
    await fs.writeFile(path.join(out, 'trace.json'), JSON.stringify(result, null, 2) + '\n');
  }
  return { file: 'trace.json', moduleSha256: result.moduleSha256, framesPerPass: 255, passes: ['first', 'warm'] };
}
