...
const before = read('before/manifest.json'), after = read('after/manifest.json');
...
for (const [id, b] of Object.entries(before.images)) {
  ...
  const files = ['before', 'after'].map((side, i) => {
...
fs.writeFileSync(path.join(dir, 'comparison.json'), JSON.stringify({
  beforeSha: before.sha, afterSha: after.sha, matchedViews: 19,
  note: 'Combined changes. Fixed time12.6 daylight survey, not an official take. Five additional threshold views have no matched baseline here.',
  views: results,
}, null, 2) + '\n');
