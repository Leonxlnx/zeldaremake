...
const pinned = [
  ...
  { source: 'e7fa081152314b592bec147541944e9fbdbfd1f5', bundle: 'index-Bq5genaK.js', sha256: 'd77564dbcf21eb12648cdb84d165cd717619a5b350ebc39c93665d24fdda8974' },
];
...
async function loadCapture(settingFile, side) {
  ...
  const captureRoot = path.resolve(path.dirname(settingsPath), '../../..');
  const settings = await read(settingsPath), dir = path.resolve(captureRoot, settings.out), manifestPath = path.join(dir, 'manifest.json');
  ...
  assert.equal(hash(await fs.readFile(path.resolve(captureRoot, settings.dist, 'assets', expected.bundle))), expected.sha256, 'Pinned bundle bytes changed');
  const html = await fs.readFile(path.resolve(captureRoot, settings.dist, 'index.html'), 'utf8');
