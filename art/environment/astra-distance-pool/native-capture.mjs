...
const report={sha:settings.sourceSha,settings,images:{},errors:[],evidenceKind:'Native combined baseline/candidate: fresh route then fixed views; owner view reconstructed'};
...
const server=await serveStatic(dist);let browser;const bootStarted=performance.now();
try {
 ...
 report.bootReadyMs=performance.now()-bootStarted;
 if(settings.transition)report.transition=await (await import('./matched-transition.mjs')).captureTransition(page,out);
 ...
 for(const id of settings.views??[...views,'sky-opening']) {
  ...
  for(const variant of settings.variants??[{name:'default'}]) {
   ...
   report.images[name]={variant,sha256:crypto.createHash('sha256').update(png).digest('hex'),...await page.evaluate(()=>({stats:__ZR__.stats(),camera:__ZR__.cameraPose(),lighting:__ZR__.audit().systems.lighting,distantClose:__ZR__.audit().systems.trees?.distantClose,pool:__ZR__.perf().systemPerf.trees,nearCanopy:__ZR__.audit().systems.trees?.nearCanopy}))};
