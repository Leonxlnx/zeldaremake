...
| Tall ("bossy kid") house | trunk (48.6, −1.6), radius 3.0, dome 7.6 m (the tallest), door facing −84° | Side deck on the south face: planks at 6.86 m (1.15 m above the door floor). A railed walk strip runs (47.25, 2.28) → (50.21, 2.18), and 5 plank steps climb from (45.59, 2.45) to (47.33, 2.40). 6 posts, 8 boards, and a 5-rung ladder at the far end, footed at (50.78, 2.28). |
...
| Lookout | rope fence on the south lip, 5 posts from (45.3, 8.85) to (50.3, 8.55); log bench at (48.3, 7.55), yaw −132°, 1.9 m long | The bench faces back over the lane. |
...
- Character ground (the live mask plus `ctx.shared.walkSurfaces`): the discs are walkable. These are walls:
  ...
  lantern posts, the bench and the deck railings. The deck and its steps are walk surfaces (`east-tall-deck`,
...
- The deck's skirt (`697e251a`): Link's feet predict the next foothold up to about 0.86 m ahead. Turning at
  ...
  Link now stops 0.80 m outside the planks' outer edge (was 0.16 m) and 0.94 m past their far end (was
  0.2–0.7 m). That takes 3.3 m² of ground he could reach before. The test asserts that the skirt opens
  nothing and only closes air over the ground. It also floods from the deck by the movement rule, down the
...
- `npx tsc --noEmit`, `npx vite build`, and every `*.test.mjs` under `src/` and `gauntlet/` (39 files): pass
  at `697e251a`.
- Merge onto the canonical head `03e1127a` (which now carries `EXPANSION_SOUTH` and the owner-23:00 merges:
  stone hero flight, kids, near veil, tree LOD). It conflicts in 10 files, every one a place where the east and
  south expansions add side by side: `layout.ts`, `heightfield.ts`, `character/ground.ts`,
  `hardscape/index.ts`, `hardscape/flagstones.ts`, `structures/index.ts`, `trees/index.ts`,
  `vegetation/expansion.ts`, `expansion2.test.mjs` and `playtest.mjs`. Resolved in a scratch worktree (not
  pushed), keeping both sides. The only non-mechanical choice: `expansionCull` keeps its `east` flag and
  passes it on to canonical's new `westExpansionCull`, and the white-barks and the understory keep east off,
  as on both branches. There, `tsc`, `vite build` and `node --test src/world/*/*.test.mjs` (99 files) pass.
  The play-mode walk on that merged build completes every route: `stairs-to-east-lookout` 20/20,
  `east-tall-deck` 8/8, `east-small-door` 5/5, canonical's `south-bridge-to-log` 21/21 (51.7 m) and
  `plaza-to-upper-house` 6/6. Nothing is stuck and there are no page errors. The east routes' camera and
  clearance numbers match this branch's.
...
### Play-mode walk (builds `38e1cd13` and `697e251a`: identical numbers)
...
route's numbers are unchanged.

### Draw calls and triangles

Probed on build `38e1cd13` against the base `746f1d39`. `697e251a` changes only the character ground, not what
is drawn.
...
### Hero frames (960×540, identical shot list, SSIM and share of pixels changed by more than 8/255)
...
- `stairs-to-east-lookout` reports a footprint corner 0.31 m below the surface. It is on the main stairway
  ...
  corner is −6.3 cm.
- The two lane views over hero A's budget numbers (`green_wide`, `lookout_back`, see the perf table).
