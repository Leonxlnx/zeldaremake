...
export function measureScreen(asset, records, origin, native, view = 'F_canopy') {
  assert.equal(native.settings.width,width);assert.equal(native.settings.height,height);
  const pose=native.images[view].camera,cam=new T.PerspectiveCamera(pose.fov,width/height,.1,500);
  ...
  const coreFloor=new Uint8Array(count),bareCore=new Uint8Array(count),missingFine=new Uint8Array(count);
  ...
  for(let i=0;i<far.index.count;i+=3) {
    ...
    const horizontalFloor=core&&group===26&&ids.every(id=>Math.abs(far.attributes.position.getY(id)-records.get(26).floorY)<1e-6);
    ...
    raster(ids.map(id=>farPoints[id]),(at,z)=>{
      if(z<depth[at]){depth[at]=z;if(core){coreOwner[at]=group;coreFloor[at]=Number(horizontalFloor);}}
  ...
  let fineUnion=0,fineUnoccluded=0,frontmostGroup26FloorMaskPixels=0,fineInFrontOfFloorPixels=0;
  const floorFineInFrontByGroup={24:0,25:0,26:0},floorSampleCoords=[];
  for(let at=0;at<count;at++) {
    ...
    if(Number.isFinite(coreDepth[at])){
      overall.projectedCorePixels++;increment(overall,at);increment(groups.get(coreOwner[at]).coverage,at);
      bareCore[at]=Number(coreDepth[at]<=Math.min(fineDepth[at],oldLeafDepth[at],woodDepth[at]));
      missingFine[at]=Number(fineDepth[at]>=Math.min(coreDepth[at],oldLeafDepth[at],woodDepth[at]));
      if(coreFloor[at]){
        frontmostGroup26FloorMaskPixels++;
        if(fineDepth[at]<coreDepth[at]-1e-8){fineInFrontOfFloorPixels++;floorFineInFrontByGroup[fineOwner[at]]++;}
        if(frontmostGroup26FloorMaskPixels%400===1)floorSampleCoords.push([at%width,Math.floor(at/width)]);
      }
    }
  ...
  const regions=(mask,group)=>{
    const seen=new Uint8Array(count),stack=new Int32Array(count),out=[];
    for(let first=0;first<count;first++){
      if(!mask[first]||seen[first]||(group!==undefined&&coreOwner[first]!==group))continue;
      let sp=1,pixels=0,minX=width,minY=height,maxX=0,maxY=0;stack[0]=first;seen[first]=1;
      while(sp){
        const at=stack[--sp],x=at%width,y=Math.floor(at/width);pixels++;minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);
        for(const next of [x?at-1:-1,x<width-1?at+1:-1,y?at-width:-1,y<height-1?at+width:-1])
          if(next>=0&&!seen[next]&&mask[next]&&(group===undefined||coreOwner[next]===group)){seen[next]=1;stack[sp++]=next;}
      }
      out.push({pixels,bounds:[minX,minY,maxX,maxY]});
    }
    out.sort((a,b)=>b.pixels-a.pixels);return{components:out.length,largest:out.slice(0,8)};
  };
  const contiguous={connectivity:4,frontmostCore:regions(bareCore),withoutFrontmostFine:regions(missingFine),groups:[24,25,26].map(group=>({group,frontmostCore:regions(bareCore,group)}))};
  const rayChecks=[],checkCoords=view==='F_canopy'?[[675,85],[1000,155],[733,122]]:[[52,65],[85,205],[180,145]];
  checkCoords.push(...floorSampleCoords);
  for(let y=20;y<340;y+=40)for(let x=view==='F_canopy'?560:20;x<(view==='F_canopy'?1140:460);x+=40)checkCoords.push([x,y]);
  ...
  return {view,width,height,time,pose,groups:[...groups.values()],overall,leafMetrics,fineUnionPixels:fineUnion,fineUnoccludedByBankOpaquePixels:fineUnoccluded,trianglePixelSamples,contiguous,
    outsideCoreMask:{fineUnionPixels:fineUnion-overall.anyFineProjection,fineUnoccludedPixels:fineUnoccluded-overall.frontmostFine,summedLeafFootprints:leafMetrics.summedDistinctLeafFootprintPixels-leafMetrics.summedLeafFootprintsInsideCore,leavesWhollyOutsideMask:leaves.filter(l=>l.pixels>0&&l.corePixels===0).length},
    floor:{group:26,floorLocal:records.get(26).floorY,originY:origin.y,floorWorld:records.get(26).floorY+origin.y,frontmostGroup26FloorMaskPixels,fineInFrontOfFloorPixels,floorFineInFrontByGroup,floorSampleCoords},
    coreMaskSha256:crypto.createHash('sha256').update(Buffer.from(coreDepth.buffer)).digest('hex'),
