/**
 * Kokiri Forest cinematic (opus-cinematic-b-sept25, adapted from opus-cinematic-sept24): ORIGINAL offline sound design.
 *
 ...
 *   detail   pod-lantern flame (breath + husk + crackle/pops), macro leaf ticks, and (optional
 *            'hamlet' section) two distant stick–slip timber creaks in the tree houses.
 *   steps    the game's footstep voices at the supplied contact times + cloth/gear foley.
 *   music    the game score, started so phrase B's high G lands on the cue, with gain/low-pass
 *            automation for the rise, plus an original harp glissando, swell and low bloom.
 *
 * Every curve is a per-section table (retime by editing cues.json sections only); the optional
 * 'hamlet' section has an entry in each, and a section the cut lacks just drops out.
 */
...
const SR = 48000;
const TAU = Math.PI * 2;
/** the score's beat: render-audio.mjs derives it from music.ts (LOOP_SECONDS / 64) and passes cfg.beat */
let BEAT = 60 / 76;
...
/**
 * Breakpoints from a per-section table { id: [[anchor 'start'|'end', offset s, value], …] }: every
 * point is placed relative to its own section, so a retimed cut moves it, and a section the cut
 * does not have (e.g. the optional 'hamlet') simply drops out — the curve then glides between its
 * neighbours. `over` (cues.json `curves.<name>`) replaces a section's list, or removes it with null.
 */
function sectionPoints(S, table, over) {
  const merged = { ...table, ...(over ?? {}) };
  const pts = [];
  for (const [id, list] of Object.entries(merged)) {
    if (!S[id] || !Array.isArray(list)) continue;
    for (const [anchor, off, v] of list) pts.push([(anchor === 'end' ? S[id].end : S[id].start) + off, v]);
  }
  return pts;
}
/** the last edit second the cut covers (sections sorted by start) */
const cutEnd = (S) => Math.max(...Object.values(S).map((s) => s.end));
...
/**
 * The placed calls, each keyed to its section by an offset from the section's start. A call whose
 * section is missing from the cut, or which would start in the last 0.1 s of a (shortened) section,
 * is dropped rather than spilling into the next shot.
 */
const BIRD_PLAN = [
  // hero opening: far birds across a wide field, one higher voice as the dolly settles
  ['hero', 0.25, 'blackbird', 0.72, -0.55, 0.55],
  ['hero', 0.8, 'woodpigeon', 0.93, 0.42, 0.8],
  ['hero', 1.75, 'chiffchaff', 0.78, 0.64, 0.42],
  ['hero', 2.6, 'robin', 0.86, -0.28, 0.4],
  ['hero', 3.5, 'goldcrest', 0.62, 0.22, 0.32],
  // lantern close-up: one bird near the pods, behind the flame (at d 0.16 / 0.34 it measured
  // −19 LUFS momentary, 8 dB over the lantern it is meant to sit behind, and ran over the macro)
  ['lantern', 0.12, 'robin', 0.35, 0.36, 0.1],
  // canopy reveal: higher and farther
  ['canopy', 0.35, 'greattit', 0.62, -0.38, 0.45],
  ['canopy', 1.25, 'goldcrest', 0.55, 0.22, 0.38],
  ['canopy', 1.95, 'wren', 0.68, 0.56, 0.5],
  // walk
  ['walk', 0.7, 'blackbird', 0.6, 0.52, 0.4],
  ['walk', 2.6, 'chiffchaff', 0.7, -0.6, 0.4],
  ['walk', 4.4, 'wren', 0.76, 0.42, 0.4],
  // run
  ['run', 0.8, 'chaffinch', 0.62, 0.6, 0.45],
  ['run', 2.5, 'woodpecker', 0.86, -0.66, 0.55],
  // stairs
  ['stairs', 0.45, 'woodpigeon', 0.95, -0.5, 0.75],
  ['stairs', 1.7, 'woodpecker', 0.9, 0.55, 0.55],
  ['stairs', 2.9, 'thrush', 0.7, 0.35, 0.42],
  // hamlet (optional section): a small grove of tree houses — a near robin, a thrush across the
  // grove, one contact chip; nothing after 2.6 s, where the score's swell and glissando take over
  ['hamlet', 0.3, 'robin', 0.55, -0.4, 0.4],
  ['hamlet', 1.5, 'thrush', 0.8, 0.55, 0.36],
  ['hamlet', 2.6, 'chip', 0.62, -0.2, 0.3],
  // final reveal: the chorus opens up, then settles
  ['reveal', 0.22, 'blackbird', 0.55, -0.45, 0.45],
  ['reveal', 1.05, 'wren', 0.62, 0.5, 0.42],
  ['reveal', 1.9, 'chiffchaff', 0.72, -0.2, 0.4],
  ['reveal', 2.6, 'greattit', 0.6, 0.3, 0.4],
  ['reveal', 3.15, 'woodpigeon', 0.9, 0.62, 0.65],
  ['reveal', 4.15, 'robin', 0.58, -0.5, 0.38],
  ['reveal', 4.85, 'chaffinch', 0.72, 0.15, 0.32],
  ['reveal', 5.9, 'blackbird', 0.86, 0.4, 0.4],
];
function birdPlan(S) {
  return BIRD_PLAN.filter(([id, off]) => S[id] && S[id].start + off < S[id].end - 0.1).map(([id, off, sp, d, pan, level]) => ({ sp, at: S[id].start + off, d, pan, level, section: id }));
}
...
/**
 * section value table → smooth curve (0.15 s blends either side of each boundary). `over` (a
 * cues.json `curves.<name>` table) replaces single sections; a section with no value anywhere (a
 * new id, or a missing optional one) glides between its neighbours.
 */
function sectionCurve(S, bySection, blend = 0.15, extra = [], over) {
  const table = { ...bySection, ...(over ?? {}) };
  const pts = [];
...
const CHORUS_RATE = { hero: 1.0, lantern: 0.35, macro: 0.3, canopy: 0.85, walk: 0.75, run: 0.65, stairs: 0.75, hamlet: 0.9, reveal: 1.5 };
const CHORUS_LEVEL = { hero: 1, lantern: 0.55, macro: 0.5, canopy: 1, walk: 0.9, run: 0.85, stairs: 0.9, hamlet: 0.95, reveal: 1.1 };
const RUSTLE_DENSITY = { hero: 1, lantern: 0.25, macro: 0.2, canopy: 1.25, walk: 0.9, run: 0.9, stairs: 0.9, hamlet: 1.0, reveal: 1.2 };
/**
 * How closed the sky is over each shot (the game's AmbienceState.canopy, index.ts surfaceAt: ~1 on
 * the north forest floor, 0 on the plaza / village flagstones where the player shots were scouted).
 * It sets how much of the game's canopy roll survives (1 − 0.45 · (1 − canopy)), how dark its top
 * is and how much goes to the hall. Override per section with cues.json curves.canopy.
 */
const CANOPY_BY_SECTION = { hero: 0.7, lantern: 0.4, macro: 0.6, canopy: 1, walk: 0.15, run: 0.15, stairs: 0.25, hamlet: 0.85, reveal: 0.5 };
...
function birdsJS(cfg, n, root) {
  ...
  const cv = cfg.curves ?? {};
  const rate = sectionCurve(S, CHORUS_RATE, 0.15, [[cutEnd(S) - 1.2, 0.8]], cv.chorusRate);
  const lvl = sectionCurve(S, CHORUS_LEVEL, 0.15, [], cv.chorusLevel);
...
// ---------------------------------------------------------------------------------------------
/**
 * The designed gust (0..1, the game's wind.uGust), per section as [anchor, offset, value]: the
 * hero breathes, the close-ups go still, the canopy tilt rolls one big swell through the crowns,
 * the player shots sit in a light moving air, the hamlet lifts gently and the reveal opens on a
 * second swell that settles as the fade comes in. Retime-safe (sectionPoints); cues.json
 * curves.gust replaces any section's list.
 */
const GUST_TABLE = {
  hero: [['start', 0, 0.58], ['start', 2.2, 0.7], ['end', -0.35, 0.62]],
  lantern: [['start', 0.15, 0.22]],
  macro: [['end', -0.2, 0.18]],
  canopy: [['start', 0.2, 0.3], ['start', 1.9, 0.96], ['end', -0.6, 0.86]],
  walk: [['start', 0.7, 0.42], ['end', -1.0, 0.46]],
  run: [['start', 0.5, 0.52], ['end', -0.5, 0.5]],
  stairs: [['start', 0.5, 0.45], ['end', -0.7, 0.55]],
  hamlet: [['start', 0.6, 0.5], ['end', -1.0, 0.64]],
  reveal: [['start', 1.0, 0.9], ['start', 2.2, 0.82], ['start', 4.6, 0.5]],
};
function gustCurve(S, over) {
  return curve([[-6, 0.55], ...sectionPoints(S, GUST_TABLE, over), [cutEnd(S) + 10, 0.45]]);
}
/** how open the wind bed is: close-ups pull it in (the camera is inside a lantern / on moss) */
const INTIMACY_TABLE = {
  lantern: [['start', -0.1, 1], ['start', 0.1, 0.34]],
  macro: [['start', -0.08, 0.34], ['start', 0.12, 0.27], ['end', -0.08, 0.27]],
  canopy: [['start', 0.2, 0.7], ['start', 1.5, 1.0]],
  walk: [['start', -0.1, 1.0], ['start', 0.15, 0.82]],
  run: [['start', -0.1, 0.82], ['start', 0.2, 0.9]],
  stairs: [['start', 0, 0.9]],
  hamlet: [['start', 0.25, 0.95]],
  reveal: [['start', -0.3, 0.95], ['start', 0.6, 1.08]],
};
function intimacyCurve(S, over) {
  return curve([[-6, 1], ...sectionPoints(S, INTIMACY_TABLE, over), [cutEnd(S) + 10, 1.0]]);
}
...
function leafRustles(cfg, bus, root, gustFn, intimFn) {
  const S = cfg.S;
  const density = sectionCurve(S, RUSTLE_DENSITY, 0.15, [], cfg.curves?.rustle);
...
function bedJS(cfg, n, root) {
  const S = cfg.S;
  const bus = stereoBus(n);
  const gustFn = gustCurve(S, cfg.curves?.gust);
  const intimFn = intimacyCurve(S, cfg.curves?.intimacy);
...
/**
 * A timber creak — a tree house's boards and lashings flexing in the moving air: stick–slip
 * friction, a train of tiny slips whose rate rises and falls (and now and then stutters), each
 * ringing three narrow wood modes. Returned mono, peak-normalised; `place()` puts it in the grove.
 */
function creak(r) {
  const dur = R(r, 0.45, 0.95);
  const N = Math.round(dur * SR);
  const tail = Math.round(0.1 * SR);
  const out = new Float32Array(N + tail);
  const modes = [[R(r, 300, 440), 9, 1], [R(r, 760, 980), 11, 0.5], [R(r, 1450, 1950), 12, 0.22]].map(([f, q, a]) => ({ bp: new Biquad('bp', f, q), a }));
  const r0 = R(r, 16, 26);
  const r1 = r0 * R(r, 1.8, 2.8);
  let next = 0;
  for (let i = 0; i < N + tail; i++) {
    let x = 0;
    if (i < N && i >= next) {
      const u = i / N;
      const env = u < 0.25 ? smooth(u / 0.25) : smooth((1 - u) / 0.75);
      x = env * (0.55 + 0.45 * r());
      const rate = r0 + (r1 - r0) * Math.sin(Math.PI * Math.min(1, u * 1.15));
      next = i + Math.round((SR / rate) * (1 + (r() * 2 - 1) * 0.2) * (r() < 0.08 ? 2 : 1));
    }
    let y = 0;
    for (const m of modes) y += m.bp.run(x) * m.a;
    out[i] = y;
  }
  let peak = 0;
...
/** hamlet creaks: [offset from the section start, distance, pan, relative level] — two, far apart, soft */
const CREAKS = [[0.9, 0.55, 0.35, 1], [2.25, 0.7, -0.3, 0.7]];

function detailJS(cfg, n, root) {
  const S = cfg.S;
  const bus = stereoBus(n);
  const E = (e) => Math.round((e + cfg.pre) * SR);
  const stats = { crackles: 0, ticks: 0, creaks: 0 };
  // ---- hamlet (optional section): a couple of distant timber creaks in the tree houses ----
  if (S.hamlet && cfg.creak > 0) {
    const kr = root.fork('creak');
    CREAKS.forEach(([off, d, pan, rel], i) => {
      if (S.hamlet.start + off >= S.hamlet.end - 0.2) return;
      // a slip train is mostly silence between clicks (crest ~20 dB): 0.22 of peak puts the nearer
      // creak ~15 dB under the grove's bed + birds — heard in a gap, never a feature
      place(creak(kr.fork(`k${i}`)), E(S.hamlet.start + off), d, pan, 0.22 * rel * cfg.creak, bus);
      stats.creaks++;
    });
  }
  if (!S.lantern || !S.macro) return { bus, stats };
...
function foleyJS(cfg, n, root) {
  ...
  cfg.steps.forEach((ev, idx) => {
    ...
    // cfg.foley (cues levels.foley, 0.8): the foley bypasses the game's sfx compressor and trim, so
    // against the compressed steps it would otherwise sit ~2 dB hotter than it was balanced at
    const amp = 0.03 * k * cfg.foley * R(r, 0.8, 1.2);
    for (let i = 0; i < len; i++) {
    ...
    if (idx % 2 === 0 || r() < 0.3) {
      ...
      const ga = 0.012 * k * cfg.foley * R(r, 0.7, 1.2);
...
function ornamentsJS(cfg, n, root, offset) {
  ...
  // low bloom under the hit, a fifth over the score's own root. Phrase B now brings its own G2 pad
  // in on the downbeat (music.ts PHRASE_PAD_BEATS: G2 97.999 Hz + G3), so the old G2 / G3 bloom
  // sat on the pad's partials and beat against them (~0.09 Hz) for eleven beats. D3 / D4 fill the
  // chord (G B D) without touching the pad; the detuned twin is closer (null ~8 s, past the decay).
  const s0 = E(hit);
  const bloom = [[146.83, 0.085], [293.66, 0.03]];
  const L = Math.round(4.5 * SR);
  for (let i = 0; i < L && s0 + i < n; i++) {
    const t = i / SR;
    const env = Math.min(1, t / 0.18) * Math.exp(-t / 2.3);
    let y = 0;
    for (const [f, a] of bloom) y += a * (Math.sin(TAU * f * t) + Math.sin(TAU * f * 1.0004 * t)) * 0.5;
    y *= env * cfg.bloom;
    if (s0 + i >= 0) { bus.L[s0 + i] += y; bus.R[s0 + i] += y; bus.sL[s0 + i] += y * 0.3; bus.sR[s0 + i] += y * 0.3; }
  }
  // where the ornaments sit, in edit seconds (provenance: they must land just before the hit)
  const r3 = (x) => +x.toFixed(3);
  return { bus, timing: { glissando: [r3(hit - BEAT), r3(hit)], swell: [r3(hit - 2.6), r3(hit)], bloom: [r3(hit), r3(hit + 4.5)] } };
}

/**
 * The score's rise, from cues.json `music` (all optional):
 *   enterSection  where the score starts being felt (default 'canopy'; silent before it)
 *   gain / lowpassHz   { <section>: value } reached at the END of each section before the hit
 *   preHitGain / preHitHz   0.35 / 0.4 s before the hit (this replaces the value of the section that
 *                  leads into the hit, e.g. the hamlet: its whole length is the last build)
 *   hitGain, afterGain, floorHz, openHz   the bloom on the downbeat and the carry to the end
 * Sections missing from the cut drop out; a new section with no value glides between neighbours.
 */
function musicCurves(S, hit, m = {}) {
  const order = Object.keys(S).sort((a, b) => S[a].start - S[b].start);
  const enter = S[m.enterSection ?? 'canopy'] ?? S[order[0]];
  const G = { canopy: 0.25, walk: 0.32, run: 0.42, stairs: 0.52, ...(m.gain ?? {}) };
  const F = { canopy: 3000, walk: 4200, run: 5200, stairs: 6500, ...(m.lowpassHz ?? {}) };
  const floorHz = m.floorHz ?? 1400;
  const openHz = m.openHz ?? 17000;
  const afterGain = m.afterGain ?? 0.76;
  const g = [[-10, 0], [enter.start - 0.35, 0]];
  const f = [[-10, floorHz], [enter.start, floorHz]];
  for (const id of order) {
    const s = S[id];
    // only sections that end after the score enters and well before the hit's own ramp
    if (s.end <= enter.start + 0.2 || s.end > hit - 0.75) continue;
    if (typeof G[id] === 'number') g.push([s.end, G[id]]);
    if (typeof F[id] === 'number') f.push([s.end, F[id]]);
  }
  g.push([hit - 0.35, m.preHitGain ?? 0.66], [hit + 0.05, m.hitGain ?? 0.8], [hit + 3, afterGain], [hit + 60, afterGain]);
  f.push([hit - 0.4, m.preHitHz ?? 10000], [hit + 0.1, openHz], [hit + 60, openHz]);
  return { gain: curve(g), lowpass: curve(f, 'log'), points: { gain: g, lowpassHz: f } };
}
...
window.renderStem = async (name, cfg) => {
  const S = cfg.S;
  if (typeof cfg.beat === 'number') BEAT = cfg.beat;
  ...
  const forestReturn = (level, dest = ctx.destination) => {
    const cv = ctx.createConvolver();
    cv.buffer = forestIR(ctx, createRng(cfg.seed).fork('forest-ir'));
    const ret = gain(ctx, level);
    cv.connect(ret).connect(dest);
    return cv;
  };
  /** a JS-rendered bus (dry + forest-IR send) into `dest` (default the stem's output) */
  const addBus = (bus, wetLevel, dest = ctx.destination) => {
    addBuffer(bus.L, bus.R, dest);
    addBuffer(bus.sL, bus.sR, forestReturn(wetLevel, dest));
  };
  ...
  switch (name) {
    case 'bed': {
      ...
      const gustFn = gustCurve(S, cfg.curves?.gust);
      const canopyFn = sectionCurve(S, CANOPY_BY_SECTION, 0.15, [], cfg.curves?.canopy);
      paramCurve(trim.gain, intimacyCurve(S, cfg.curves?.intimacy), ctxSeconds, pre);
      // the phase1 ambience state: how closed the sky is per shot (the roll's level, top and hall),
      // the world's wind direction (src/world/wind/wind.ts: (0.72, −0.69), so the roll leans a
      // little upwind), no tunnel, no gorge. The canopy leads by 0.6 s: its filter glides at 0.35 s,
      // its level at 0.9 s. The listener faces one fixed way — the lean is a colour, not a pan.
      const amb0 = cfg.ambience ?? {};
      const windDir = amb0.windDir === null ? undefined : amb0.windDir ?? { x: 0.722, z: -0.692 };
      const forward = amb0.forward ?? { x: 0, z: -1 };
      let canopySum = 0;
      let ticks = 0;
      for (let t = 0; t < ctxSeconds; t += 1 / 20) {
        const canopy = clamp(canopyFn(t - pre + 0.6), 0, 1);
        amb.update(t, { gust: gustFn(t - pre + 0.8), listener: { x: 0, y: 1.2, z: 2 }, forward, pods: [], canopy, windDir, enclosure: 0, gorge: 0 });
        if (t >= pre) { canopySum += canopy; ticks++; }
      }
      const { bus, stats } = bedJS(cfg, n, root);
      addBus(bus, 0.55);
      Object.assign(info, stats, { gameBed: cfg.gameBed, meanCanopy: +(canopySum / Math.max(1, ticks)).toFixed(3), windLean: amb.stats?.().windLean ?? null });
      break;
    }
    case 'birds': {
    ...
    case 'steps': {
      const buses = createBuses(ctx, root.fork('buses'));
      // phase1's sfx bus is sfx → compressor (−30 dB, 4:1) → SFX_TRIM → master, so a gain on
      // buses.sfx would be a change of how hard the compressor works, not a level. The per-section
      // step gain (cues steps.gainDb) rides AFTER the master instead — a plain dB offset on the
      // compressed steps, their hall return and the foley alike.
      buses.master.disconnect();
      const post = gain(ctx, 1);
      buses.master.connect(post).connect(ctx.destination);
      paramCurve(post.gain, (e) => {
      ...
      addBus(bus, 0.35, post);
      info.steps = cfg.steps.length;
      info.foley = count;
      info.surfaces = cfg.steps.reduce((m, ev) => ((m[ev.surface] = (m[ev.surface] ?? 0) + 1), m), {});
      break;
    }
    case 'music': {
      const buses = createBuses(ctx, root.fork('buses'));
      ...
      const hit = cfg.musicHit;
      const mc = musicCurves(S, hit, cfg.music ?? {});
      paramCurve(auto.gain, mc.gain, ctxSeconds, pre);
      paramCurve(lp.frequency, mc.lowpass, ctxSeconds, pre);
      // cues music.scoreDb: the score's level over the game's own balance (0.45 into a music bus
      // 12 dB under the ambience). Phase1's score breathes (PHRASE_LEVEL, one pad per phrase), so at
      // the game's balance it sits ~40 dB under full scale — right for a wood you walk around in,
      // inaudible under a trailer's forest. Dry and hall send move together.
      const scoreGain = 0.45 * Math.pow(10, ((cfg.music ?? {}).scoreDb ?? 0) / 20);
      const musicOut = gain(ctx, scoreGain);
      const musicReverb = gain(ctx, scoreGain);
      musicOut.connect(buses.music);
      musicReverb.connect(buses.reverb);
      // startAt from render-audio.mjs (score-derived): phrase B's first note lands on cfg.musicHit
      // however late the hit is — for a late hit the score simply starts later in the context
      const startAt = cfg.musicStartAt ?? 0.5;
      const music = createMusic(ctx, musicOut, musicReverb, root.fork('score'), startAt, false);
      info.musicSource = await music.ready;
      music.scheduleUntil(ctxSeconds);
      const { bus, timing } = ornamentsJS(cfg, n, root, pre);
      info.ornaments = timing;
      info.automation = mc.points;
      ...
      ss.start(0);
      const hitBeat = cfg.hitBeat ?? 32;
      const phraseBRender = startAt + (cfg.scoreLeadIn ?? 0.3) + hitBeat * BEAT;
      info.phraseB = { startAt, beat: hitBeat, renderSeconds: +phraseBRender.toFixed(5), offsetSeconds: pre, editSeconds: +(phraseBRender - pre).toFixed(5), scoreStartsAtEdit: +(startAt + (cfg.scoreLeadIn ?? 0.3) - pre).toFixed(5) };
      break;
    }
    default:
