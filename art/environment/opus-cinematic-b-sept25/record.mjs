 ...
 *
 *   node art/environment/opus-cinematic-b-sept25/record.mjs --shots shots.json --out gauntlet/out/opus-cinematic-b-sept25/take1
...
import { ROOT, serveStatic, findChrome, openWorld } from '../../../gauntlet/scripts/lib/browser.mjs';

/**
 * browser.mjs's launchBrowser (same flags, native D3D11 under ZR_NATIVE_GPU=1) plus a persistent
 * profile, so Chrome's GPU program cache survives between runs: with a fresh temp profile every
 * launch recompiled every shader variant (≈ 4–5 min on the first frame of each new view on the
 * Radeon 780M). The cache only changes compile time, never the pixels.
 */
async function launchBrowser({ width, height, profile }) {
  ...
  return puppeteer.launch({
    ...
    userDataDir: profile,
    args: [
      '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-gpu-sandbox', '--use-gl=angle',
      `--use-angle=${nativeGpu ? 'd3d11' : 'swiftshader'}`, ...(nativeGpu ? [] : ['--enable-unsafe-swiftshader']),
      '--ignore-gpu-blocklist', '--enable-webgl', '--enable-webgl2-compute-context', '--hide-scrollbars', '--mute-audio',
      '--gpu-disk-cache-size-kb=2097152', `--window-size=${width},${height}`,
    ],
...
const out = path.resolve(typeof args.out === 'string' ? args.out : path.join(ROOT, 'gauntlet/out/opus-cinematic-b-sept25/take'));
...
const EASES = {
  ...
  easeIn: (t) => t * t * t,
  // moving from the first frame (v0 = 2x the mean), constant deceleration to rest on the last frame
  easeOutQuad: (t) => 1 - (1 - t) * (1 - t),
  // cubic Hermite: already moving at the mean speed on frame 0, a soft peak (1.33x) at u = 1/3, finite deceleration to rest
  easeOutHermite: (t) => t + t * t - t * t * t,
};
...
function loadShots(file) {
  ...
  const plan = list.map((s, idx) => {
    ...
    if (kind === 'camera') {
      ...
      return { name: s.name, kind, s: s.s, frames, startFrame, settle, keys, ease: easeName, time, timeExplicit: s.time !== undefined, atmo: s.atmo ?? null, hide: Array.isArray(s.hide) ? s.hide : null, drive: s.drive === 'direct' ? 'direct' : 'pose' };
    }
    ...
    return {
      name: s.name, kind, s: s.s, frames, startFrame, settle, time, timeExplicit: s.time !== undefined, atmo: s.atmo ?? null, hide: Array.isArray(s.hide) ? s.hide : null, drive: s.drive === 'direct' ? 'direct' : 'pose',
...
function installPageRecorder() {
  ...
  const baseRender = composer ? composer.render : null;
  const noop = () => {};
  if (!P || !hero || !charGroup) throw new Error(`recorder: player handle / link / character group missing (${!!P}/${!!hero}/${!!charGroup})`);
  /**
   * Hiding without changing the light count (opus-cinematic-b): toggling `character` /
   * `background-characters` visibility takes Navi's and the kids' fairies' PointLights out of the
   * scene, which changes NUM_POINT_LIGHTS and recompiles every lit program (≈ 4 min per switch on
   * the 780M; the codebase avoids it itself, npc.ts:689-694). Instead, for the one render call the
   * groups' drawables are hidden and their lights dimmed to 0, then restored, so the systems see
   * exactly the state they set and every shot shares the boot-time (warmup=1) programs.
   */
  const hide = { char: false, cast: true };
  const hideFor = (root, hid, dim) => root.traverse((o) => {
    if (o.isLight) { dim.push([o, o.intensity]); o.intensity = 0; }
    else if ((o.isMesh || o.isSprite || o.isPoints || o.isLine) && o.visible) { hid.push(o); o.visible = false; }
  });
  const realRender = composer ? function (...a) {
    const hid = [], dim = [];
    if (hide.cast && cast) hideFor(cast, hid, dim);
    if (hide.char) hideFor(charGroup, hid, dim);
    try {
      return baseRender.apply(this, a);
    } finally {
      for (const o of hid) o.visible = true;
      for (const [o, i] of dim) o.intensity = i;
    }
  } : null;
  if (composer) composer.render = realRender;
  ...
  const groundAt = (x, z) => {
    // the live terrain, the ground the character walks (phase1: decks, grove flight, spans) and the analytic main-village treads
    let h = Math.max(H.terrain.height(x, z), P.groundHeight ? P.groundHeight(x, z) : -Infinity, P.surfaceHeight ? P.surfaceHeight(x, z) : -Infinity);
  ...
  const R = {
    ...
    groundAt,
    /**
     * 'pose': __ZR__.setPose — every system's onCameraMove reset path each frame (LOD a pure function of the pose, no
     * hysteresis). 'direct': the first frame of a shot goes through setPose, later frames move __H.camera itself, so
     * the systems' own per-frame camera checks run with their hysteresis / travel gates (capture.md §4).
     */
    setCam(pose, drive, first) {
      if (drive !== 'direct' || first) { Z.setPose(pose.p, pose.t, pose.fov); return; }
      const c = H.camera;
      c.position.set(pose.p[0], pose.p[1], pose.p[2]);
      c.up.set(0, 1, 0);
      c.lookAt(pose.t[0], pose.t[1], pose.t[2]);
      if (pose.fov && c.fov !== pose.fov) { c.fov = pose.fov; c.updateProjectionMatrix(); }
      c.updateMatrixWorld();
    },
    ...
    async setupPlayer(spec, startXZ) {
      ...
      // phase1's PlayerHandle.place(x, z, yaw) is the yaw setter: Link starts at `start` facing `face`,
      // or else the first moving input's direction (the preroll then only has to reach a steady gait).
      // It replaces opus-cinematic-sept24's turn-in-place input (magnitude 0.06 engaged the walk and
      // crept him ~0.07 m/s while he turned).
      const firstMove = spec.inputs.find((i) => Math.hypot(i.moveX, i.moveZ) > 0.05);
      const yaw0 = spec.face ? Math.atan2(spec.face[0], spec.face[1]) : firstMove ? Math.atan2(firstMove.moveX, firstMove.moveZ) : P.heading();
      if (typeof P.place !== 'function') throw new Error('PlayerHandle.place missing');
      P.place(startXZ[0], startXZ[1], yaw0);
    ...
    async frame(S, ticks, capture, u, warm) {
      ...
      R.setCam(pose, S.spec.drive, !S.posed);
      S.posed = true;
      ...
      return {
        ...
        reachClamped: !!a.linkIk?.reachClamped, npcsVisible: hide.cast ? 0 : a.npcsVisible, linkSource: a.linkSource,
        charVisible: charGroup.visible && !hide.char, contextLost: !!window.__zrContextLost,
      };
    },
  };
  window.__REC = R;
...
async function main() {
  ...
  const profile = path.resolve(typeof args.profile === 'string' ? args.profile : path.join(ROOT, 'gauntlet/out/opus-cinematic-b-sept25/chrome-profile'));
  const browser = await launchBrowser({ width, height, profile });
  let aborted = null;
  ...
  try {
    ...
    for (const s of selected) {
      ...
      const shotReport = { name: s.name, kind: s.kind, run: runId, startFrame: s.startFrame, frames: s.frames, time: s.time, atmo: s.atmo, hide: s.hide };
      // per-shot composer overrides (composer.ts reads __ATMO_SETTINGS__ every frame), e.g. {fanMix: 0.4} on pans
      // …and per-shot hidden objects by name (composer.ts __ATMO_HIDE__: every pass incl. shadows), e.g. ["falling-leaves"]
      // when a closed-form falling leaf crosses right in front of a low lens
      await page.evaluate(([a, h]) => { window.__ATMO_SETTINGS__ = a; window.__ATMO_HIDE__ = h; }, [s.atmo, s.hide]);
      if (s.kind === 'camera') {
        ...
        for (const i of idx) {
          ...
          const r = await page.evaluate(async ([pose, setT, warm, dt, drive]) => {
            if (setT !== null) window.__ZR__.setTime(setT);
            window.__REC.setCam(pose, drive, setT !== null);
            ...
            const g = window.__REC.groundAt(pose.p[0], pose.p[2]);
            return { simTime: st.simTime, triangles: st.triangles, calls: st.drawCalls, cam: window.__ZR__.cameraPose(), contextLost: !!window.__zrContextLost, clearanceM: pose.p[1] - g };
          }, [pose, first ? want - 1 / fps : null, first ? s.settle - 1 : 0, 1 / fps, s.drive]);
          prevT = i;
          ...
          if (r.clearanceM < 0.3) console.error(`${s.name} frame ${i}: CAMERA ${r.clearanceM.toFixed(2)} m over the ground`);
          saveFrame(s, i, u, buf, { simTime: +r.simTime.toFixed(5), cameraPose: { set: round(pose), actual: round(r.cam) }, clearanceM: +r.clearanceM.toFixed(3), triangles: r.triangles, calls: r.calls, retries, contextLost: r.contextLost }, Date.now() - t0);
        }
      } else {
        const spec = {
          name: s.name, time: s.time, prerollTicks: s.prerollTicks, simHz: SIM_HZ, face: s.player.face, inputs: s.player.inputs, fullTicks, drive: s.drive,
          camera: s.camera,
        };
