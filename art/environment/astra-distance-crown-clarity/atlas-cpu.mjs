...
const baselineRef = process.argv[2] || '664f3418';
const beforeSource = execFileSync('git', ['show', `${baselineRef}:${file}`], { encoding: 'utf8' });
const afterSource = readFileSync(file, 'utf8').replaceAll('\r\n', '\n');
assert.ok(afterSource.split('export function createFarCrownAtlas(')[0] === beforeSource.split('export function createFarCrownAtlas(')[0], 'ordinary/near leaf painters and atlas layout are unchanged (checkout line endings ignored)');
const before = compile(beforeSource);
const after = compile(afterSource);
...
const evidence = resolve('art/environment/astra-distance-crown-clarity/cpu');
