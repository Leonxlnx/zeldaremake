...
const views = [
  { id: 'w05-spine-d', p: [0, 1.45, 0.56], t: [0, 0.01, -1.94], fov: 46 },
  { id: 'w16-spine-d', p: [2.87, 3.78, -32.24], t: [3.29, 2.98, -34.71], fov: 46 },
  ...['A_stairs', 'B_house', 'C_lookback', 'D_log', 'E_ground', 'F_canopy'].map(id => ({ id })),
];
for (const build of builds) {
  ...
  const server = await serveStatic(build.root); let browser;
  try {
    ...
    for (const pose of views) {
      ...
      const state = await page.evaluate(() => ({ stats: __ZR__.stats(), camera: __ZR__.cameraPose(), lighting: __ZR__.audit().systems.lighting, hardscape: __ZR__.audit().systems.hardscape, failures: __ZR__.audit().systemFailures }));
