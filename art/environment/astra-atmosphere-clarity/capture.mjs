...
assert.equal(digest(settingsBytes),builds.settingsSha256);assert.equal(settings.views.length,4);
...
const report={sha:snapshot.source,snapshot,settingsSha256:builds.settingsSha256,settings,captureScriptSha256:digest(await fs.readFile(new URL(import.meta.url))),images:{},errors:[],note:'The upward pose is reconstructed, not the original owner camera. Native high 1280x720, time12.6, 14 settled frames. Static timing includes requestAnimationFrame and synchronous one-pixel read; no gameplay FPS claim.'};
...
try{
 ...
 for(const id of settings.views){
  const pose=settings.poses[id];
  await page.evaluate(async({id,pose})=>{if(pose)__ZR__.setPose(pose.p,pose.t,pose.fov);else if(id==='sky-opening')__ZR__.setPose([0,1.8,.5],[0,18,-15],72);else if(!__ZR__.setViewpoint(id))throw Error('Unknown viewpoint');__ZR__.setTime(12.6);await __ZR__.render(14,0);},{id,pose});
  const bytes=await page.screenshot({path:path.join(out,id+'.png')});
  const info=await page.evaluate(()=>{const a=__ZR__.audit();return {stats:__ZR__.stats(),camera:__ZR__.cameraPose(),lighting:a.systems.lighting,atmosphere:a.systems.atmosphere,perf:__ZR__.perf()};});
  ...
  report.images[id]={sha256:digest(bytes),...info};
  ...
  if(id==='reconstructed-distant-up'||id==='A_stairs'){
