...
const report={sha:settings.sourceSha,settings,images:{},errors:[],evidenceKind:'Native fixed A/F pair; baseline root623 reused with identical independently rebuilt bundle'};
...
try {
 ...
 for(const id of settings.views??[...views,'sky-opening']) {
  ...
  for(const variant of settings.variants??[{name:'default'}]) {
   ...
   report.images[name]={variant,sha256:crypto.createHash('sha256').update(png).digest('hex'),...await page.evaluate(()=>{const a=__ZR__.audit(),gl=window.__probeRenderer.getContext(),ext=gl.getExtension('WEBGL_debug_renderer_info');return {stats:__ZR__.stats(),camera:__ZR__.cameraPose(),lighting:a.systems.lighting,vegetation:a.systems.vegetation,renderer:ext?gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER)};})};
