#!/usr/bin/env node
/**
 * walkby.mjs — walk past the birds and ask whether any of them noticed.
 *
 *   node art/audio/2026-09-25-parallax/walkby.mjs --dist dist --tag before [--out /tmp/parallax]
 *
...
const log = (...m) => console.error('[walkby]', ...m);
  ...
  for (const t of TAKES.takes) {
    ...
    const r = await page.evaluate(
      async (from, to, speed, secs, g, ld, lp) => {
        ...
        return { wav: btoa(b), spots: o.bed?.birdSpots ?? [], perches: o.bed?.perchSpots ?? [], birds: o.bed?.birds ?? 0 };
      },
      t.from,
      t.to,
      t.speed,
    ...
    done.push({ ...t, len, seconds, birds: r.birds, spots: r.spots, perches: r.perches });
    log(`${t.id} — ${t.note} (${len.toFixed(1)} m at ${t.speed} m/s), ${r.birds} calls, perches ${r.perches.map((p) => p[0]).join(' ')}`);
  }
