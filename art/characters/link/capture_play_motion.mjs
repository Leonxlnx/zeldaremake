...
try{
  ...
  const flatVideo=process.argv.includes('--flat-video')||walkVideo;
  ...
  for(const scenario of jumpOnly?['run-jump']:process.argv.includes('--stairs-only')?['stairs-up','stairs-down']:(flatVideo||flatStills||process.argv.includes('--flat-only'))?['flat-transitions']:descentDetail?['stairs-down']:['flat-transitions','stairs-up','stairs-down']){
    ...
    for(let start=0;start<frames;start+=chunk){
      ...
      const rows=await page.evaluate(async({scenario,start,count,stairDetail,walkVideo})=>{
        ...
        for(let j=0;j<count;j++){
          ...
          player.setInput({moveX:moving?d[0]:0,moveZ:moving?d[1]:0,run:jumping||!walkVideo&&scenario==='flat-transitions'&&i>=120,jump:jumping&&i===45});
      ...
      },{scenario,start,count,stairDetail,walkVideo});
  ...
  if(flatVideo){
    ...
    const file=path.join(out,walkVideo?'walk-idle.mp4':'walk-run-idle.mp4');
