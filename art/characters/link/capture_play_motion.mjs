...
try{
  ...
  await page.goto(server.url+'/?capture=1&dev=0&hud=0&quality=high'+(candidate==='link-runtime.glb'?'':'&link='+encodeURIComponent(candidate)),{waitUntil:'domcontentloaded',timeout:180000});
  ...
  for(const scenario of (flatVideo||flatStills||process.argv.includes('--flat-only'))?['flat-transitions']:descentDetail?['stairs-down']:['flat-transitions','stairs-up','stairs-down']){
    ...
    const chunk=flatVideo||flatStills?2:descentDetail?10:30;
    for(let start=0;start<frames;start+=chunk){
      ...
      const rows=await page.evaluate(async({scenario,start,count})=>{
        ...
        for(let j=0;j<count;j++){
          ...
          const row={scenario,frame:i,gait:c.linkGait,root,placement,feet:c.linkFeetContact,ik:c.linkIk,
