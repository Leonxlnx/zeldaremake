...
for side in [-1,1]:
    sleeve=sweep('Short sleeve '+str(side),[(side*.121,0,.853),(side*.158,0,.835),(side*.193,0,.791),(side*.209,0,.760)], [.048,.061,.070,.076],cloth,aspect=.95,sides=32,subdiv=2,caps=False)
...
for side in [-1,1]:
    x=side*.056;z=1.068;cy=face_y(x,z)+.026
    def eye_surface(xx,zz):
        return cy-math.sqrt(max(.00005,.038**2-xx*xx-zz*zz))
    def aperture(a,r=1):
        xx=.031*math.cos(a)*r
        zz=(.0142 if math.sin(a)>=0 else .017)*math.sin(a)*(.82+.18*abs(math.sin(a)))*r+side*xx*.085
        return xx,zz
    ...
    eye_disc('Iris rim ',.0174,irisrim,.00035)
    eye_disc('Iris teal ',.0164,iris,.0006)
    eye_disc('Pupil ',.0078,pupil,.00085)
    ...
    for upper in [True,False]:
        ...
        for row in range(5):
            ...
            for j in range(33):
                ...
                xx,zz=aperture(a)
                xx+=.005*t*math.cos(a);zz+=(.010 if upper else .006)*t*math.sin(a)
...
for k in range(rows):
    for j in range(cols):
        a=math.tau*j/cols;end=1.93+.36*max(0,math.sin(a))+.09*math.sin(3*a+.4)
...
for i,pts in enumerate(locks):
    rad=[.030,.045,.042,.036,.032,.032][i] if i<6 else .023
    sweep('Hair | swept layered clump %02d'%i,pts,[rad*.50,rad,rad*.72,.001],hairmats[1+i%3],aspect=.45,sides=16,subdiv=2)
for i in [1,3]:
