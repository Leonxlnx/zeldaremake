...
The motion source remains the author's [Quaternius Universal Animation Library, CC0](https://quaternius.com/packs/universalanimationlibrary.html), `Jog_Fwd_Loop`, with the existing phase match and repository provenance in `reference/animations/quaternius-standard/SOURCE.json`. The raw source was independently smooth under the same normalized-cycle check. No new motion service or asset license was added. This does not relicense the separate character model or claim that the entire project is CC0.
