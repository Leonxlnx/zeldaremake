...
const out = path.join(ROOT, 'art/characters/link/progress/2026-09-21-run-carriage');
process.env.ZR_NATIVE_GPU = '0';
const names = ['baseline-delivery3056.glb', 'rebuilt-run-candidate.glb'];
const files = [path.join(ROOT, 'art/characters/link/progress/2026-09-21-color-bake/stairs-upright-delivery.glb'), path.join(out, names[1])];
...
assert.equal(hash(bytes[0]), '305603e92277952f345842e526216ff066b2b5888908eef054990199b6597a69');
assert.equal(hash(bytes[1]), '89df38f255e47afbcbb28a60555fb4a4a20091741d427ef1d7b8ea1ac33f306b');
...
try {
  ...
  await fs.writeFile(path.join(out,'rebuilt-comparison.json'),JSON.stringify(report,null,2)+'\n');
