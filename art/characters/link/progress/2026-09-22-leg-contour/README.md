...
**Held study, not the default model.** The native export is `a70ef714`; raw preservation, native correspondence and unchanged-foot replay pass. The additional remodeled-region check finds new calf entries into timber. Retained new entries are 0.064 and 0.324 mm; a retained existing intersection deepens by 0.454 mm. The earlier apparent 49.8 mm case lies below the timber underside and is not solid penetration. Complete classification of all changed contacts is in progress. The accepted game asset remains `4dcf89c5`. See [export verification](EXPORT.md) and [current contact evidence](CONTACT.md).
