...
for attempt in range(32):
 ...
 for tri in m.loop_triangles:
  ...
  if old.length>1e-10 and (old.dot(new)<=0 or not .1<=new.length/old.length<=10):bad+=1
 if not bad:break
 scale*=.8
assert not bad,bad
''' +code[b:]
...
code=code.replace('asymmetric fringe shape study','whole hair stronger fringe study')
code=code.replace('fringe-shape','fringe-strong')
