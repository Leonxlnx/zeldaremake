...
for loop in loops:
    ...
    for i in loop:
        ...
        zz=(.014 if math.sin(angle)>=0 else .019)*math.sin(angle)*(.82+.18*abs(math.sin(angle)))+side*xx*.085
...
for side in [-1,1]:
    ...
    def aperture(a,r=1):
        xx=.037*math.cos(a)*r
        zz=(.014 if math.sin(a)>=0 else .019)*math.sin(a)*(.82+.18*abs(math.sin(a)))*r+side*xx*.085
        return xx,zz
    ...
    def eye_disc(name,radius,mat,offset):
        ...
        for k in range(1,7):
            for j in range(steps):
                ...
                limit=(.014 if zz>=0 else .019)*math.sqrt(max(0,1-(xx/.037)**2))
