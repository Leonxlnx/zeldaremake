...
def material(name, color, roughness=.6, metallic=0, texture=None):
    ...
    if texture:
        ...
        if texture == 'leather':
            ...
            bump.inputs['Strength'].default_value=.35; bump.inputs['Distance'].default_value=.0012
...
for side in [-1,1]:
    ...
    curve('Boot welt seam '+s,[(x+xx*.965,yy*.965,.032) for xx,yy in outline],.0023,edge,True)
    toe=ellipsoid('Boot toe '+s,(x,-.055,.064),(.061,.085,.037),leather)
    ...
    curve('Toe cap seam '+s,[(x-.054,-.080,.067),(x-.035,-.11,.087),(x,-.12,.091),(x+.035,-.11,.087),(x+.054,-.08,.067)],.001,edge)
    for n in range(4):
        z=.115+n*.019
        y=-.070+n*.004
        for k in [-1,1]:
...
def face_profile(z):
    ...
    for axis in range(1,4):
        ...
        values.append(max(.001,.5*((2*b)+(-a+c)*t+(2*a-5*b+4*c-d)*t*t+(-a+3*b-3*c+d)*t*t*t)))
    return values

def face_y(x,z):
    rx,front,_=face_profile(z)
    y=-front*math.sqrt(max(.005,1-(x/rx)**2))
...
for side in [-1,1]:
    ...
    ellipsoid('Iris rim '+str(side),(x,ey-.001,z),(.0128,.0018,.0133),irisrim,48,20)
    ellipsoid('Iris teal '+str(side),(x,ey-.0026,z),(.0118,.0012,.0123),iris,48,20)
    for k in range(70):
        a=math.tau*(k+rng.uniform(-.25,.25))/70
        r1=rng.uniform(.0062,.009);r2=rng.uniform(.010,.012)
    ...
    ellipsoid('Pupil '+str(side),(x,ey-.004,z),(.0059,.001,.0064),pupil,32,20)
...
for i,pts in enumerate(locks):
    ...
    sweep('Hair | tapered lock %02d'%i,pts,[rad*.55,rad,rad*.60,.001],hairmats[1+i%3],aspect=.65,sides=12,subdiv=2)
    # Three slender ridges run with each lock and remain separate editable curves.
    for strand in range(-5,6):
        line=[(x+strand*rad*.10,y-.013*(1-k/len(pts)),z+.001) for k,(x,y,z) in enumerate(pts)]
