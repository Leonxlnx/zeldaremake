...
def hair_lock(name,points,width,mat):
    ...
    return sweep(name,centres,radii,mat,aspect=.12,sides=8,subdiv=1)
for i,pts in enumerate(locks):
    rad=[.026,.027,.030,.022,.025,.025][i] if i<6 else .020
    layers=5 if i<6 else 3
    for strand in range(layers):
        spread=(strand/(layers-1)-.5)*1.5
        path=[]
        for j,(x,y,z) in enumerate(pts):
            fan=.35 if j==0 else 1.0
            path.append((x+rad*spread*fan,
                         y-.003*math.cos(strand*1.7+i)-.0015*strand,
                         z+.005*math.sin(strand*2.1+i)*j/3))
        hair_lock('Hair | layered ribbon %02d %02d'%(i,strand),path,
                  rad*(.34 if i<6 else .43),hairmats[1+(i+strand)%3])
for i in [1,3]:
...
def child_z(z):
    ...
    return .851+(z-.90)*.90
def smooth_step(a,b,value):
    t=max(0,min(1,(value-a)/(b-a)))
    return t*t*(3-2*t)

boot_seating={ob.name:max(0,min((ob.matrix_world@v.co).z for v in ob.data.vertices)-.022)
              for ob in hero.objects if ob.name.startswith('Boot | continuous instep')}
assert len(boot_seating)==2, 'Expected both continuous leather uppers'
...
def proportion(p,ob):
    ...
    if ob in head_parts:
        ...
        p.x*=.93
    # Author rest shaping before reduction and baking so both surfaces agree.
    x,y,z=p
    if ob.name.startswith(('Hair |','Eyebrow hair','Upper lash')):
        amount=.12*smooth_step(.012,.070,y)*(1-smooth_step(1.02,1.145,z))
        if amount:p.z=max(z-amount,.864+.009*math.sin(x*90))
    if ob.name.startswith('Cap |'):
        p.z+=.048*smooth_step(.025,.11,-y)*(1-smooth_step(1.085,1.16,z))
        drape=smooth_step(.08,.18,y)*(1-smooth_step(1.10,1.20,p.z))
        p.x*=1+.75*drape
        p.z-=.025*drape*(1-smooth_step(.93,1.03,p.z))
    if ob.name in boot_seating:
        p.z-=boot_seating[ob.name]*(1-smooth_step(.045,.105,z))
    return p
...
for name in boot_seating:
    ob=bpy.data.objects[name]
    low=min((ob.matrix_world@v.co).z for v in ob.data.vertices)
    assert abs(low-.022)<1e-5, ('Leather upper misses its welt',name,low)
scene['rest_shapes_authored']=True
...
bpy.data.libraries.write(str(ROOT/'link-study.blend'),{scene},fake_user=True,compress=True)
