...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const out={stoneTriangles:stone.triangles,timberTriangles:timber.triangles,stoneGrid:plain.surfaceInfo().stairs,fullGrid:full.surfaceInfo().stairs,queries:0,timberSamples:0,stoneOnlyMaxDeltaM:0,heightMaxDeltaM:0,maxTimberUnderestimateM:0,worst:null,normalBands:{},misses:0};
    for(let step=0;step<def.steps;step++)for(const across of [-def.width/4,0,def.width/4])for(const offset of [-.08,-.06,-.04,-.02,0,.02,.04,.06,.08]){
      ...
      if(!hit||hit.object.name!=='stairs-main-logs'||hit.face.normal.y<.1)continue;
      out.timberSamples++;
  ...
  const file='art/characters/link/progress/2026-09-21-motion-integration/timber-shoulder-'+(candidate?'candidate':'raw')+'.json';
