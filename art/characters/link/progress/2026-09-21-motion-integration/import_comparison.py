...
assert hashlib.sha256(asset.read_bytes()).hexdigest() == EXPECTED_SHA
name = SCENE_NAME
...
(out / (LABEL + '-import.json')).write_text(json.dumps(report, indent=2), encoding='utf-8')
bpy.data.libraries.write(str(out / (LABEL + '-native.blend')), {scene}, fake_user=True, compress=True)
