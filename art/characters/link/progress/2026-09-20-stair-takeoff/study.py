...
job=globals().get('JOB',{});stem=job.get('stem','stairs-lower-swing')
...
scene=loaded.scenes[0];scene.name=job.get('scene','Link | September19 lower stair swing');bpy.context.window.scene=scene
...
frames=176;fps=240;factor=frames/track.strips[0].action_frame_end;lift_scale=job.get('lift_scale',.06/.145)
...
advance=job.get('forward_advance',0)
assert 0<=advance<1/math.pi,'Swing time remapping must remain monotonic'
if advance:
    original=[{s:(p.copy(),pitch) for s,(p,pitch) in pair.items()} for pair in samples]
    for f,pair in enumerate(samples):
        for side,(p,pitch) in pair.items():
            offset=0 if side=='L' else .5;phase=(f/frames+offset)%1
            if phase<=.48:continue
            t=(phase-.48)/.52;early=.48+.52*(t+advance*math.sin(math.pi*t)**2)
            frame=((early-offset)%1)*frames;lo=int(frame);w=frame-lo
            p.y=original[lo][side][0].y*(1-w)+original[lo+1][side][0].y*w
...
loop=max(abs(a-b) for name in ends[0] for a,b in zip(ends[0][name],ends[1][name]));assert loop<1e-5,(loop,str(samples[0]),str(samples[-1]),[(n,max(abs(a-b) for a,b in zip(ends[0][n],ends[1][n]))) for n in ends[0]])
for f in curves():
...
report={'scene':scene.name,'gait':'stairs','frames':frames,'fps':fps,'cycle_s':frames/fps,'stride_m':.8066667,'preserve_hips':True,'lift_scale':lift_scale,'forward_advance':advance,'bone_error_m':error,'loop_error':loop,'other_curves_exact_at_normalized_phase':True,'status':'Native candidate; actual shoe and world checks required'}
