...
const out = path.join(ROOT, 'art/characters/link/progress/2026-09-21-motion-research');
const source = path.join(ROOT, 'art/characters/link/progress/2026-09-21-motion-integration');
const names = ['stairs-upright-candidate.glb', 'run-torso-candidate.glb'];
const bytes = await Promise.all(names.map(n => fs.readFile(path.join(source,n))));
...
for (const [i,a] of base.doc.animations.entries()) {
  ...
  if(a.name!=='run') { assert.deepEqual(a,b); continue; }
...
assert.deepEqual(changed,[['chest','rotation'],['head','rotation']]);
...
try {
  ...
  report.results=await page.evaluate(async names=>{
    const [{loadGlbLink},{createLocomotion},{hardChain},{Vector3,Quaternion,AnimationMixer},{GLTFLoader}]=await Promise.all([
    ...
    const summarize=rows=>{
      ...
      const result={samples:rows.length,metrics,armOppositeThigh};
      if(rows[0].feet) {
    ...
    for(const mode of ['native','capture','play']) {
      ...
      for(const name of names) {
        ...
        if(mode==='native') {
          const gltf=await new GLTFLoader().loadAsync(url),mixer=new AnimationMixer(gltf.scene),clip=gltf.animations.find(a=>a.name==='run');
          const action=mixer.clipAction(clip);action.play();
          for(let i=0;i<=112;i++) { action.time=clip.duration*i/112;mixer.update(0);gltf.scene.updateMatrixWorld(true);rows.push(snapshot(gltf.scene)); }
          specs.push({durationS:clip.duration,samples:113});
        } else {
          const puppet=await loadGlbLink(url),chain=hardChain('run'),loco=mode==='play'?createLocomotion():null;
          const dt=1/60,speed=mode==='play'?4.6:3.9;let pose;
          for(let i=0;i<720;i++) {
          ...
          const clip=puppet.asset.clips.find(a=>a.name==='run');
      ...
      const result={base:summarize(a),candidate:summarize(b),specs,zeroDt:zeros,maxHeadOrientationDifferenceDeg:maxHead,
  ...
  await fs.writeFile(path.join(out,'composed-comparison.json'),JSON.stringify(report,null,2)+'\n');
  ...
  for(const [mode,r] of Object.entries(report.results)) {
    ...
    if(mode==='play') { assert.equal(r.candidate.reachClamps,0);assert.ok(r.candidate.flightFraction>0);
