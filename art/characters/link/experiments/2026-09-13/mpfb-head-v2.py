...
macro.update(age=.17,gender=1.0,muscle=.3,weight=.4)
...
def fit(p):
    ...
    socket=math.exp(-3*((dx/.055)**2+(dz/.060)**2)**2)*(1-smooth(-.025,.025,y))
    x+=(1 if x>0 else -1)*dx*.35*socket
    z+=dz*.85*socket
    lower=math.exp(-((z-.904)/.054)**4)*(1-smooth(.01,.075,y))
    x*=1-.10*lower
    z+=.022*lower
    return Vector((x,y,z))
eye_geometry=[]
...
head=bpy.data.objects.new('CC0 MPFB Link head v2',geometry);scene.collection.objects.link(head)
...
for side,verts,faces in eye_geometry:
    data=bpy.data.meshes.new('V2 eye '+side);data.from_pydata(verts,[],faces);data.update()
    ob=bpy.data.objects.new('V2 eye '+side,data);scene.collection.objects.link(ob)
...
bpy.data.libraries.write('E:/Tools/blender-mcp/mpfb-link-v2.blend',{scene},fake_user=True,compress=True)
