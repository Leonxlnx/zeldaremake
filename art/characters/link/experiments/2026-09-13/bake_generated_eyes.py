...
body_colour_name={'material-candidate':'scanned-body-color','hardware-candidate':'hardware-body-color','face-smooth-candidate':'hardware-body-color','eyelash-candidate':'hardware-body-color','textured-iris-candidate':'hardware-body-color','corneal-candidate':'hardware-body-color'}.get(stem,'body-eye-edit-color')
eye_colour_name='corneal-eye-color' if stem=='corneal-candidate' else ('textured-iris-color' if stem=='textured-iris-candidate' else ('anatomical-eye-color' if stem in {'eye-candidate','eye-depth-candidate','iris-plane-candidate'} else 'iris-material-color'))
bakes.update(body_color=bake_input(body,body_colour_name,4096),
    eye_color=bake_input(eyes[0],eye_colour_name,2048 if stem in {'textured-iris-candidate','corneal-candidate'} else 1024))
for eye in eyes:
...
triangle_limit=60000 if stem in {'connected-lid-candidate','eyelash-candidate','textured-iris-candidate','corneal-candidate'} else (55000 if folder=='source-runtime' else 27000)
...
if stem=='corneal-candidate':
    assert not any('clearcoatNormalTexture' in m.get('extensions',{}).get('KHR_materials_clearcoat',{}) for m in gltf['materials'])
    record['source_candidate_sha256']=hashlib.sha256((root/'textured-iris-candidate.glb').read_bytes()).hexdigest()
    record['corneal_study']=json.loads((root/'corneal-surface-v2-study.json').read_text())
