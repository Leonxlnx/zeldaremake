...
for side in ['l','r']:
    key=TargetService.load_target(human,str(ROOT/f'src/mpfb/data/targets/eyes/{side}-eye-scale-incr.target.gz'),weight=1.0)
    key.slider_max=3.0;key.value=2.7
for side in ['l','r']:
...
head=bpy.data.objects.new('CC0 MPFB Link head v6',geometry);scene.collection.objects.link(head)
...
for side,verts,faces in eye_geometry:
    data=bpy.data.meshes.new('V6 eye '+side);data.from_pydata(verts,[],faces);data.update()
    ob=bpy.data.objects.new('V6 eye '+side,data);scene.collection.objects.link(ob)
...
bpy.data.libraries.write('E:/Tools/blender-mcp/mpfb-link-v6.blend',{scene},fake_user=True,compress=True)
