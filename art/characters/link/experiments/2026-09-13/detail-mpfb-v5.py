...
scene=bpy.data.scenes['MPFB | Link proportions v5'];bpy.context.window.scene=scene
head=bpy.data.objects['CC0 MPFB Link head v5']
...
def surface(x,z):
    ...
    return hit[0].y-.0006
brown=bpy.data.materials.new('V5 brow umber');brown.diffuse_color=(.19,.080,.022,1);brown.use_nodes=True
...
for side in [-1,1]:
    for i in range(28):
        ...
        data=bpy.data.curves.new('V5 eyebrow fibre','CURVE');data.dimensions='3D';data.bevel_depth=.0007*(1-.65*t);data.bevel_resolution=2;data.resolution_u=5
        ...
        ob=bpy.data.objects.new('V5 eyebrow fibre',data);scene.collection.objects.link(ob);data.materials.append(brown)

for side in ['l','r']:
    old=bpy.data.objects['MPFB v5 iris '+side]
    ...
    data=bpy.data.meshes.new('V5 iris radial geometry');data.from_pydata(verts,[],faces);data.update()
    ...
    mat=bpy.data.materials.new('V5 teal radial iris');mat.use_nodes=True;ns=mat.node_tree.nodes;ls=mat.node_tree.links
    ...
    ob=bpy.data.objects.new('V5 detailed iris '+side,data);scene.collection.objects.link(ob);data.materials.append(mat)
    for p in data.polygons:p.use_smooth=True
    scene.collection.objects.unlink(old)
    pupil=bpy.data.objects['MPFB v5 pupil '+side];pupil.location.y=centre.y-.0027
head['detail_pass']=True
scene.render.filepath='E:/Tools/blender-mcp/mpfb-link-v5-detail.png'
bpy.ops.render.render(write_still=True)
bpy.data.libraries.write('E:/Tools/blender-mcp/mpfb-link-v5-detail.blend',{scene},fake_user=True,compress=True)
