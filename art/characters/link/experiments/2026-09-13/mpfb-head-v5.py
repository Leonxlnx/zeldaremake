...
def fit(p):
    ...
    # Sum the two smooth fields: symmetry keeps the facial midline fixed.
    dx_total=0.0;dz_total=0.0
    for side in [-1,1]:
        dx=x-side*.048;dz=z-1.002
        socket=math.exp(-((dx/.047)**2+(dz/.045)**2))*(1-smooth(-.025,.025,y))
        dx_total+=dx*.30*socket
        dz_total+=dz*.65*socket
    x+=dx_total;z+=dz_total
    lower=math.exp(-((z-.904)/.064)**2)*(1-smooth(.01,.075,y))
    x*=1-.07*lower
    z+=.022*lower
    return Vector((x,y,z))
assert abs(fit(Vector((0,eye.y,eye.z))).x)<1e-8, 'The facial midline must remain fixed'
eye_geometry=[]
for side in ['l','r']:
...
head=bpy.data.objects.new('CC0 MPFB Link head v5',geometry);scene.collection.objects.link(head)
...
for side,verts,faces in eye_geometry:
    data=bpy.data.meshes.new('V5 eye '+side);data.from_pydata(verts,[],faces);data.update()
    ob=bpy.data.objects.new('V5 eye '+side,data);scene.collection.objects.link(ob)
...
bpy.data.libraries.write('E:/Tools/blender-mcp/mpfb-link-v5.blend',{scene},fake_user=True,compress=True)
