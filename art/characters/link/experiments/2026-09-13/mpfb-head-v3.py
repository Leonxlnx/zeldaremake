...
head=bpy.data.objects.new('CC0 MPFB Link head v3',geometry);scene.collection.objects.link(head)
...
for side,verts,faces in eye_geometry:
    data=bpy.data.meshes.new('V3 eye '+side);data.from_pydata(verts,[],faces);data.update()
    ob=bpy.data.objects.new('V3 eye '+side,data);scene.collection.objects.link(ob)
...
bpy.data.libraries.write('E:/Tools/blender-mcp/mpfb-link-v3.blend',{scene},fake_user=True,compress=True)
