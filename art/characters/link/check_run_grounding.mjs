...
try{
  ...
  const result=await page.evaluate(async asset=>{
    ...
    const clip=puppet.asset.clips.find(c=>c.name==='run'),dt=clip.durationS/120;
    const loco=createLocomotion(),chain=hardChain('run');loco.speed=clip.strideM/clip.durationS;loco.dt=dt;
    const rows=[];
    for(let i=0;i<240;i++){
    ...
    return {asset,frames:rows.length,rootRangeM:Math.max(...ys)-Math.min(...ys),
