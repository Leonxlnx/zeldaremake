...
import type { forecastOrdinaryBody } from './ordinary-body-forecast';
...
export interface HandoffInput {
  ...
  dimensions:ConfigurationDimensions; ground:GroundSampler; sinceAir:number; nextBody:ReturnType<typeof forecastOrdinaryBody>;
}
...
export function planContactHandoff(previous:HandoffState|null,input:HandoffInput){
  ...
  const boundary=frame.at(dt).request;
  const stationary=boundary.soles.map(sole=>({...cloneSole(sole),velocity:new Vector3(),angularVelocity:new Vector3()})) as [SoleMotion,SoleMotion];
  const nextBoundaryHips=input.nextBody.status==='clear'?
    createOrdinaryBodyCurve(boundary,input.nextBody.request,dt,d.hipHalfWidth,d.sole).at(0,stationary).hips:null;
  ...
  while(t<dt-1e-12){
    ...
    if(start.anchored[0]&&!start.anchored[1]){
      const recovery=planSupportRecovery({configuration:start.configuration,worldHeight:start.worldHeight,soles:(last?.request??input.accepted.request).soles.map(cloneSole) as [SolePose,SolePose],
        velocities:worldVelocity,duration:T,fresh:!state.recoveryPlanned,dt,frameStart:startTime,dimensions:d,ground,
        frameAt:used=>frame.recoveryAt(startTime+used),nextBoundaryHips,nextBoundaryStatus:input.nextBody.status,
