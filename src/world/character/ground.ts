...
export function createGround(terrain: Terrain, layout: Layout, shared?: SharedGeometry): Ground {
  ...
  // (2026-09-24: and the grove's, layout NORTH_STAIRS); round 57: the ruins' stair from the outcrop
  // to the terrace and the water stair from the terrace down the retaining wall to the quay (built
  // by the ruins system, not hardscape)
  const allStairs = [...layout.stairs, ...EXPANSION_STAIRS, ...NORTH_STAIRS, EXPANSION_RUINS.stairs, EXPANSION_RUINS.waterStair];
  ...
  return {
    ...
    attachSurface(scene) {
      ...
      for (const extra of [north, expansion, south]) {
