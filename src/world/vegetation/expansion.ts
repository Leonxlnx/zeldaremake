...
import { EAST_BOX, EXPANSION, EXPANSION_BOX, EXPANSION_NPC_SPOTS, EXPANSION_ROPE_FENCES, EXPANSION_SOUTH_BOXES, EXPANSION_STAIRS, expansionSteppingStones, southBankFrameVectors, southBankPoint } from '../layout';
...
/** true when the world box [x0, z0] … [x1, z1] overlaps the expansion box, the knoll's, one of round 56's south boxes or the east lane's — where `expansionCull` can be true */
export function tileMeetsExpansion(x0: number, z0: number, x1: number, z1: number): boolean {
  ...
  return meets(EXPANSION_BOX) || meets(KNOLL_BOX) || EXPANSION_SOUTH_BOXES.some(meets) || meets(EAST_BOX);
}
