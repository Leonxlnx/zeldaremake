...
{const {HERO_FERN_ULTRA_M,ULTRA_STEM_SIDES,heroFernGeometry,heroRachisTones,makePalette}=read('vegetation/plantgeo');assert.deepEqual(a.plants.heroFerns.opts.lodDistances,[HERO_FERN_ULTRA_M,16,32]);assert.ok(HERO_FERN_ULTRA_M>=4&&HERO_FERN_ULTRA_M<=8);
  ...
  // the ultra geometry (its rachis rings: ULTRA_STEM_SIDES vertices at v = 0 and v = 1) and nowhere in the high LOD
  ...
  {const pal=makePalette(WORLD.palette),{foot,tip}=heroRachisTones(pal),lum=c=>0.3*c[0]+0.59*c[1]+0.11*c[2];
    ...
    for(const v of [0,1,2]){const ultra=heroFernGeometry(`${WORLD.seed}/hero-fern/${v}`,pal,'ultra'),high=heroFernGeometry(`${WORLD.seed}/hero-fern/${v}`,pal,'high');
      ...
      assert.ok(nf>=6*ULTRA_STEM_SIDES&&nf%ULTRA_STEM_SIDES===0,`ultra ${v}: ${nf} foot-tone vertices, whole ${ULTRA_STEM_SIDES}-sided rings`);
      assert.ok(nt>=6*ULTRA_STEM_SIDES&&nt%ULTRA_STEM_SIDES===0,`ultra ${v}: ${nt} tip-tone vertices`);
    ...
    for(const v of [0,1,2]){const ultra=fiddleheadGeometry(`${WORLD.seed}/fiddlehead/${v}`,pal,'ultra'),high=fiddleheadGeometry(`${WORLD.seed}/fiddlehead/${v}`,pal,'high');
      const nf=count(ultra,st.foot),nt=count(ultra,st.tip);
      assert.ok(nf>=2*ULTRA_STEM_SIDES&&nf%ULTRA_STEM_SIDES===0&&nt>=2*ULTRA_STEM_SIDES&&nt%ULTRA_STEM_SIDES===0,`ultra fiddlehead ${v}: ${nf} foot / ${nt} tip stalk vertices in ${ULTRA_STEM_SIDES}-sided rings`);
      assert.equal(count(high,st.foot)+count(high,st.tip),0,`high fiddlehead ${v}: flat stalk tone`);
