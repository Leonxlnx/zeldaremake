...
export function createFarCrownAtlas(rng: Rng, palette: LeafClusterPalette, size = 1024): DataTexture {
  ...
  for (let i = 0; i < FAR_CROWN_CELLS; i++) {
    ...
    const columnar = i === 3;
    const clumps: Clump[] = [];
    // body clumps: rejection-sampled inside the lobe union, denser toward the core; each is a
    // soft radial blob of 1–2 m (on a 20 m crown) with a few lighter leaflets on its upper half
    ...
    for (const c of clumps) {
      ...
      const pr = c.r * cell;
      // shadow under the clump: a darker soft blob offset down (the leaf mass over the one below)
      const under = ctx.createRadialGradient(px, py + pr * 0.45, 0, px, py + pr * 0.45, pr * 0.95);
      const dark = base.clone().multiplyScalar(0.62);
      under.addColorStop(0, css(dark, c.alpha * 0.55));
      under.addColorStop(1, css(dark, 0));
      ctx.fillStyle = under;
      ctx.fillRect(px - pr, py - pr * 0.5, pr * 2, pr * 2);
      // the clump body
      const g = ctx.createRadialGradient(px, py, 0, px, py, pr);
      g.addColorStop(0, css(base, c.alpha));
      g.addColorStop(0.5, css(base.clone().multiplyScalar(0.96), c.alpha * 0.85));
      g.addColorStop(1, css(base, 0));
      ctx.fillStyle = g;
      ctx.fillRect(px - pr, py - pr, pr * 2, pr * 2);
      // leaflets over the upper half: small lighter dabs (leaf clumps catching the sky)
      ...
      for (let k = 0; k < leaflets; k++) {
        ...
        const lg = ctx.createRadialGradient(0, 0, 0, 0, 0, ll);
        lg.addColorStop(0, css(lit, c.alpha * 0.6));
        lg.addColorStop(1, css(lit, 0));
        ctx.fillStyle = lg;
        ctx.scale(1, lw / ll);
        ctx.beginPath();
        ctx.arc(0, 0, ll, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
      }
    }
