...
export function cliffPoint(z: number, v: number, ground: Ground): { p: Vector3; c: RGB; moss: number; wet: number } {
  ...
  // tone: bleached toward the brow, greyer and damp toward the foot and by the fall; the face is
  // always turned from the sun (it faces east), so it stays pale and cool and the moss keeps to
  // its ledges and foot — spread over the face the damp shade moss read as black blotches
  const hRel = clamp((y - g) / Math.max(top - g, 0.5), 0, 1);
  const k = 1.0 + 0.14 * hRel + 0.08 * n3.noise(z * 0.8, y * 0.5);
