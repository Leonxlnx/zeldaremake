...
/** The lookout's casters: its fence posts (both runs) and bench (`groundAt` samples the terrain) */
export function eastLookoutCasters(groundAt: (x: number, z: number) => number): Caster[] {
  const L = EXPANSION_EAST.lookout;
  const out: Caster[] = [...L.fence, ...L.westRun].map((p) => ({ x: p[0], z: p[2], r: 0.5, y0: groundAt(p[0], p[2]) - 0.15, y1: groundAt(p[0], p[2]) + 1.25, shadow: true }));
