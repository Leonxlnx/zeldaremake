 ...
 * sectors (their merged sector meshes' wood, and the sectors' far-foliage batches once every lobe
 * in them is out of reach: 172 K + 113 K shadow triangles over the bridge's south end on
 * b34f66a5, where the batches escaped the rule), the village's far terrain chunks and rocks. The
 * sun stands in the north-west, so their shadows fall east and south-east of them, 30 m and more
 * from the far bank, in the haze. It is measured from the box, not the camera, so the same casters
 * are off from anywhere on the far bank and nothing switches as the camera moves there.
 */
