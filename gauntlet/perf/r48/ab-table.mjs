...
const lines = ['| view | switch | on ms (median) | off ms (median) | pair-wise Δ (median) | saving | draws off | tris off (M) |', '| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |'];
for (const file of files) {
  ...
  for (const [stage, v] of Object.entries(r.ab ?? {})) {
    ...
    lines.push(`| | ${NAMES[stage] ?? stage} | ${f1(v.onMedianMs)} | ${f1(v.offMedianMs)} | ${v.pairDeltaMedianMs > 0 ? '−' : '+'}${f1(Math.abs(v.pairDeltaMedianMs))} | ${saving == null ? '—' : `${saving >= 0 ? '−' : '+'}${Math.abs(saving).toFixed(0)} %`} | ${v.offDrawCalls ?? '—'} | ${v.offTriangles ? (v.offTriangles / 1e6).toFixed(2) : '—'} |`);
  }
}
lines.push('');
lines.push('Both arms of a pair render on the same page under the same box load (`viewstats.mjs --ab`, 12 pairs per switch, order alternated); the pair-wise Δ is the median of off − on over the pairs, negative = the switch costs that much per frame.');
