 ...
 * capture's viewpoint and simulation time, then K finished frames are timed (`gl.finish()` after each
 * `step`, so the number is the frame's GPU cost natively and SwiftShader's raster cost here). The
 ...
 *        [--isolate]  (baseline only: each system alone, its draws / triangles)
 *
 * Config names are the URL flags themselves (`baseline` = none); use `;` to separate configs when a
 * flag carries a comma (`veg=0.5,0.5`). Writes `<out>/results.json`, `<out>/TABLE.md` and the PNGs.
 * Absolute numbers on SwiftShader are CPU-bound proxies — read the deltas, not the milliseconds.
 */
...
if (!configs.includes('baseline')) configs.unshift('baseline');
...
function table(results, view) {
  ...
  const base = rows.find((r) => r.config === 'baseline');
  const lines = [];
  lines.push(`### ${view} (${width}×${height}, settle ${settle}, ${timed} finished frames, t=${simTime}s)`);
  lines.push('');
  lines.push('| config | frame ms (median) | Δ frame | issue ms | draws | Δ draws | triangles | Δ tris | ready s | PNG vs baseline |');
  lines.push('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |');
  for (const r of rows) {
    const s = r.stats ?? {};
    const t = r.timing ?? {};
    ...
    const bs = b.stats ?? {};
    const bt = b.timing ?? {};
    const png = r.config === 'baseline' ? '(reference)' : r.pngIdentical === true ? 'identical' : r.pngIdentical === false ? 'differs' : '—';
    lines.push(
      `| ${r.error ? `${r.config} (FAILED)` : r.config} | ${fmtMs(t.medianMs)} | ${r.config === 'baseline' ? '—' : fmtPct(rel(t.medianMs, bt.medianMs))} | ${fmtMs(t.issueMedianMs)} | ${s.drawCalls ?? '—'} | ${r.config === 'baseline' ? '—' : fmtPct(rel(s.drawCalls, bs.drawCalls))} | ${s.triangles?.toLocaleString?.('en-US') ?? '—'} | ${r.config === 'baseline' ? '—' : fmtPct(rel(s.triangles, bs.triangles))} | ${r.readyMs != null ? (r.readyMs / 1000).toFixed(1) : '—'} | ${png} |`,
    );
  }
  lines.push('');
  if (base?.isolate) {
    lines.push(`#### ${view} — each system alone (baseline; colour pass only, lighting kept)`);
    lines.push('');
  ...
  try {
    for (const view of views) {
      ...
      for (const config of configs) {
        const params = config === 'baseline' ? '' : config;
        ...
        try {
          const r = await measureView(browser, server.url, {
            ...
            simTime,
            timed,
            isolate: isolate && config === 'baseline',
            out: png,
            log: (m) => console.error(`  ${m}`),
          });
          r.config = config;
          r.wallMs = Date.now() - t0;
          if (config === 'baseline') basePng = fs.readFileSync(png);
          else if (basePng) r.pngIdentical = fs.readFileSync(png).equals(basePng);
          results.push(r);
          const s = r.stats ?? {};
          console.error(`  → frame ${fmtMs(r.timing?.medianMs)} ms (issue ${fmtMs(r.timing?.issueMedianMs)}), ${s.drawCalls} draws, ${s.triangles} tris, tier ${r.tier}, flags ${JSON.stringify(r.flags)}${r.pngIdentical != null ? `, png ${r.pngIdentical ? 'identical' : 'differs'}` : ''}`);
        } catch (e) {
        ...
        fs.writeFileSync(path.join(out, 'results.json'), JSON.stringify({ git, startedAt, dist, width, height, settle, timed, simTime, configs, views, results }, null, 2));
      }
    }
  } finally {
    await browser.close();
