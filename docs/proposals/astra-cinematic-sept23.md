...
**Proposed cut: 36 seconds, six shots, 16:9.** Freeze an accepted build after the current polish; use its existing warm sun, cool forest depth, lanterns, wind and leaves. Keep the grade consistent and cut on movement. This document prepares a recording; no final video or upload has been made.

Initial inspected base: `agent/astra-motion-sept21` at `4ad2fb50`, plus the exporter timing and viewport changes described below. Original A/B/D/F PNGs from [the integrated forest at 105a61d5](../../art/environment/astra-canopy-packed-integration/README.md) informed this plan. Later, a [15-frame native 1080p static preflight](../../art/environment/astra-cinematic-preflight-sept23/README.md) on frozen `cd834342` / `index-DwReWLkW.js` verified the world capture tool and reviewed all five routes' start/middle/end poses. It recommends a lower canopy tilt and shorter house approach, and holds the stair route for framing/cost review (9.29–9.64M triangles). The original route JSON below is retained for traceability, not approved for final recording. Link at native 1080p and all moving routes remain unrendered. Recheck the final frozen source before recording.

## The cut

Coordinates are metres, `[x,y,z]`, with north toward negative Z. `P` is camera position, `T` its target; FOV is vertical degrees. The world endpoints below have static start/middle/end evidence linked above; the moving routes remain unverified. A/B/D/F anchors come from [layout.ts](../../src/world/layout.ts); the house/stair moves reuse the existing [B-roll teaser](../../gauntlet/scripts/broll.mjs).
...
`--test` renders only each start. The completed preflight expanded each world shot into start, midpoint and end as fixed poses. Before the moving export, recheck the revised canopy/house route and a lower-cost stair route, then review a short moving sample. `setPose` bypasses the free camera's ground clamp. A test array needs no production changes.
...
- **Quality and format — implemented, static world 1080p checked:** both exporters now accept `--size 1920x1080`; Link retains its 1280×720 default and 30 fps output. Its size parser passes CPU checks and the encoder asserts the requested dimensions. All 15 native 1080p world stills passed the preflight; Link at 1080p and motion remain unrendered. B-roll also accepts 4K and `--fps 60`; 60 fps delivery needs matching real samples from the Link exporter too. Avoid optical interpolation. No platform upload-limit claim is made here.
- **Performance:** six fixed views at high quality submit 7.11–8.87 M triangles / 379–479 calls. The later cinematic checkpoints submit 5.78–9.64 M / 294–464; all three stair poses exceed 9 M. These are render-budget receipts, not measured moving-shot FPS or a promise of recording speed. Output frame rate is an offline sampling rate. Resolve the stair route's cost before full export.
- **Character:** the static preflight verified the world shots hide the cast; repeat that check at final capture because B-roll only warns if its scene-hook lookup fails. `--character` only preserves visibility; it does not drive the player, and capture-view placement may change with the camera. Use the actual-input harness for shot 4. Hands, clothing contacts and extreme stair knee folding remain unfinished; the flat insert does not validate stairs. A live-control alternative already exists at `?mode=play&hud=0&dev=0`: WASD/arrows, Shift run, Space jump, drag/right stick orbit; P toggles the free camera, whose 1–6 keys recall A–F.
...
Next is revised camera checkpoint review, including a lower-cost stair route, followed by a short moving sample on the final frozen build before full export. The narrow exporter changes are implemented and CPU-checked, and the initial 15-pose world preflight is complete. No spline editor, new renderer, public gameplay change or social posting is required.
