 ...
 */
export function flight(path: (t: number) => Vector3, t: number, o: { bank?: number; extraRoll?: number; worldUp?: Vector3; dt?: number } = {}): FlightState {
  const dt = o.dt ?? 1 / 30;
  ...
  const bank = Math.atan2(turn, 400) * (o.bank ?? 1) + (o.extraRoll ?? 0);
