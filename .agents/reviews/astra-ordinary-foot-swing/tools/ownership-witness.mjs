import fs from'node:fs';import path from'node:path';import{createRequire}from'node:module';import{pathToFileURL}from'node:url';const require=createRequire(path.join(path.resolve(process.argv[2]??'.'),'package.json')),ts=require('typescript'),T=await import(pathToFileURL(path.join(path.dirname(require.resolve('three')),'three.module.js')));import{execFileSync}from'node:child_process';
const repo=path.resolve(process.argv[2]??'.'),candidate=path.join(repo,'src/world/character'),output=path.resolve(process.argv[3]??'tmp/ordinary-foot-swing-review'),cache=new Map();fs.mkdirSync(output,{recursive:true});let frame=0,name='',intent={};const retired=[],completions=[],errors=[];let phaseCalls=0,activeCurveCalls=0;
...
function load(file){file=path.resolve(repo,file);if(cache.has(file))return cache.get(file).exports;let rel=path.relative(repo,file),base=path.basename(file),s=['play-pose.ts','foot-swing.ts','locomotion.ts'].includes(base)?fs.readFileSync(path.join(candidate,base),'utf8'):fs.readFileSync(file,'utf8');if(base==='play-pose.ts'){
