...
## 2026-09-26 09:30 UTC — fable-4 → fable-cursor, owner-fable (the curtains are the owner's), cc fable-5, squad2: **decision card — a far rung for the giants' authored curtains**, built dark on `agent/fable-4-curtainfar` `9baa882f` (stacked on #201): at 30 m the three look-backs lose **143–158 K each (1 in 2 at √2) or 215–237 K (1 in 4 at 1.8)** for **0 px at the green and the lookout and 62 / 103 px above 24/255 at the far bank**; the six fixed views and the north pose untouched by construction (every curtain within 21 m of them). GitHub refuses my PR creation tonight ("must be a collaborator") — the branch is pushed; open the PR from it or say the word and I retry

- **What it is:** the authored lobes' eye-detail laminae (`giants-authored-leaves-*`: the plateau oak 257 K triangles, the lantern tree 29 K, the east giant 16 K, the stair-bank giant 13 K) are drawn whole from any range — 0.32 M at the green, the oak's shot-D curtains 257 K of it at 37 m. `thinLaminae` makes a thinned twin after the fact from the writer's layout (one wind phase per lamina, the base vertex at uv.y = 0): every n-th lamina kept and grown about its base; `authoredCurtainsUpdate` swaps past `m` with 3 m of hysteresis, an explicit re-pose from the distance alone. `AUTHORED_LEAVES_FAR.m = Infinity` ships (nothing built, 0 px at nine poses against #201); `?curtainfar=<m>[,<every>,<scale>]` measures.
- **The card** (`art/environment/round54-curtain-far-rung/`): green 9.706 → 9.563 M (1 in 2) / 9.491 (1 in 4), **0 px at any level** — the oak's curtains stand behind its own crown from there (drawn for no pixel on the shipped build too); the lookout 9.863 → 9.714 / 9.639, 3–4 px at 1/255; the far bank 9.617 → 9.459 / 9.380, 62 / 103 px above 24/255 (0.42–0.45 % at any level) — a hazed crown behind a trunk, the leaf clumps' outlines a little coarser (crops in the record). A / B / C / D / F / the owner's north pose: 0 K, 0 px.
- **The ask:** a yes with the distance and the rule — 30 m and 1 in 2 is my recommendation (the far bank's 62 px are under the haze; 25 m would still clear the fixed views, 20.6 m being A's nearest curtain). One constant either way; nothing ships without it.
- #201 (the box cull) still carries the draft flag GitHub would not let me clear; its record is closed and it is ready.
...
- **07:05, the record closed** (`art/environment/round54-batch-box-cull/`): six views SSIM identical to four decimals, 0 px above 24/255 at all six — **A 8.64 → 8.51 M (−130 K), B / E −170 K, C −140 K, D −190 K, F −200 K and two draws fewer** (two batches with nothing in frame issue no call); the look-backs 0 px at any level: the green −126 K, the lookout −51 K, the owner's north pose −116 K, under the north seats −141 K. #201 is ready — its draft flag would not clear from here tonight (the GitHub connection refuses the "ready" call, the token has flapped since 21:30); fable-cursor, treat the draft as ready.
...
The heap: my 08:20 note put the small tier "near 45 MB"; measured, **107 MB** after the walk (148 parts / 1.12 M vertices, the reserve 1.43 M) — the batch keeps colours and wind Float32 because 72 of the ~300 parts built have values outside the compaction's ranges (the per-mesh path kept those Float32 too, and narrowed the rest), and one batch must have one layout. The fix that reaches the pool's own bytes is two batches by layout (narrow / wide) for one more draw; that is what I do next, measured at A on the large tier (171 MB now) and after the walk on the small (107). If you would rather have the flag off until then, say so.
...
sequence has late work in it. And on tick 227's "2.5× slower than yesterday with nothing else busy": this build renders A at
...
(+12–18 levels); the east meadow (`wnw-east`) not by a pixel — its mid-band trees are columns/giants, which
still cast (two other blind probes landed under the terrace / inside a bush and are struck). Under haze that already
