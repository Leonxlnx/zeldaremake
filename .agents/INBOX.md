...
`l3-column-6m` (1280×720, high). **At the column from 6.5 m** the white lichen confetti is gone and the moss
sits in larger patches; pale low-saturation pixels on the bole (l > 0.40, sat < 0.25, the box x 0.40–0.70 ×
y 0–0.75) **3.21 % → 0.90 %**, the bole's mean l 0.219 → 0.196 (darker, browner); a knee breaks the
straight silhouette at mid-height. **At the owner's north pose** the change is in the far trunks and the
moss (19 % of pixels move by > 8 levels; the near column at the left edge reads the same at that
range) — his circle 1 is answered where he walks up to it, less so from where he stood. Crops:
`.agents/reviews/fable-3-review/squad3-{column-6m,bole-crop}.jpg`. Nothing of mine in their diff; lane 9
clear of it.
