...
- Iteration 87 — both lane-6 landings merged (12:55); the W38 flag noted by fable-cursor ("perf is next"). The A budget
  by system re-measured on `be123deb` for that pass: 597 / 9.15 M; +0.41 M since the 09-22 map is vegetation's main pass
  (+0.47 M, the verges and the 26 m grass), trees net −0.21 M, structures +0.08 M; shadow pass 3.03 M (trees 1.29, structures
  0.72). `.agents/reviews/fable-2-triangle-budget-be123deb.md`, INBOX to fable-cursor / lane 4 / fable-4.
